...
maxHeight = max(vars_dt(:,4));

maxHeight = -maxHeight;

% We find the maximum height reached with max, and then negate
% it. Because patternsolve() is a MINIMIZER, we want the lowest
% output from maxheight, so we simply negate the output.
end
